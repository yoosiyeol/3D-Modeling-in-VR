...
CvPoint mHand::getColorPoint(CvPoint A)
{
	...
	ColorPoint.x = (ColorSize.width / DepthSize.width)*A.x + 50;
	ColorPoint.y = (ColorSize.height / DepthSize.height)*A.y - 20;
...
void mHand::blob2Contour(const cv::Mat BlobImage, cv::Mat *OutContourImage)
{
	...
	//-----------------------ROI Left Right Hand --------------------//
	cv::Mat leftHandBlob = TempBlobImage(cv::Rect(0, 0, DepthSize.width/2, DepthSize.height));
	cv::Mat rightHandBlob = TempBlobImage(cv::Rect(DepthSize.width/2, 0, DepthSize.width/2, DepthSize.height));
...
void mHand::run(cv::Mat *Color,cv::Mat *blob)
{
	find_convex_hull(Color, RHHull, RH_contourP, LHHull_st, RHDefects_st, RHTips, &RHand_center,1);
	find_convex_hull(Color, LHHull, LH_contourP, LHHull_st, LHDefects_st, LHTips, &LHand_center,0);
...
void mHand::optHandPoint()
{
	for (int i = 0; i < 5; i++) {
		if (RHTips[i].x != 0 && RHTips[i].y != 0) {
			RHTips[i].x = ((ColorSize.width / DepthSize.width)*RHTips[i].x) + ColorSize.width / 2 + 60;
			RHTips[i].y = (ColorSize.height / DepthSize.height)*RHTips[i].y - 10;
		}
		if (LHTips[i].x != 0 && LHTips[i].y != 0) {
			LHTips[i].x = (ColorSize.width / DepthSize.width)*LHTips[i].x + 60;
			LHTips[i].y = (ColorSize.height / DepthSize.height)*LHTips[i].y - 10;
		}
		
	}
	LHand_center.x = ((ColorSize.width / DepthSize.width)*LHand_center.x) + 60;
	LHand_center.y = ((ColorSize.height / DepthSize.height)*LHand_center.y) - 10;
	RHand_center.x = ((ColorSize.width / DepthSize.width)*RHand_center.x) + (ColorSize.width / 2) + 60;
	RHand_center.y = ((ColorSize.height / DepthSize.height)*RHand_center.y) - 10;


}

CvPoint mHand::find_convex_hull(cv::Mat *Color, CvSeq *hull,CvSeq *contourP,CvMemStorage *hull_st,CvMemStorage *defects_st,CvPoint *tips,CvPoint *hand_center,int check)
{
	CvSeq *defects;
	...
	for (int i = 0; i < countdefects; i++)
	{
		...
		if (degree < 100)
		{
			...
			if (distance > 5) {
				...
				if (check == 1) {
					ColorHandCenter.x += ColorSize.width / 2 + 10;
					ColorPoint.x += ColorSize.width / 2 + 10;
				}
				tips[j] = ColorPoint;
